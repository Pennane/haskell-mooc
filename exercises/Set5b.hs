...
treeMax t = treeMax' t 0

treeMax' :: Tree Int -> Int -> Int
treeMax' Empty acc = acc
treeMax' (Node v l r) acc = max (treeMax' l newMax) (treeMax' r newMax)
  where
    newMax = if acc > v then acc else v
...
allValues pred (Node v l r) = pred v && allValues pred l && allValues pred r
...
cull t (Node v l r)
  | v == t = Empty
  | otherwise = Node v (cull t l) (cull t r)
...
isOrdered (Node v l r) = isLeftOrdered l v && isRightOrdered r v

isLeftOrdered Empty _ = True
isLeftOrdered (Node v l r) p =
  v < p
    && isLeftOrdered l v
    && isRightOrdered r v

isRightOrdered Empty _ = True
isRightOrdered (Node v l r) p =
  v > p
    && isLeftOrdered l v
    && isRightOrdered r v
...
set _ _ Empty = Empty
set [] n (Node _ l r) = Node n l r
set (StepL : steps) n (Node v l r) = Node v (set steps n l) r
set (StepR : steps) n (Node v l r) = Node v l (set steps n r)
...
search' t (Node v l r) p
  | v == t = Just p
  | otherwise = case search' t l (p ++ [StepL]) of
      Just path -> Just path
      Nothing -> search' t r (p ++ [StepR])
