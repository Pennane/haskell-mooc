...
firstHalf s = take half s
  where
    half
      | (even . length) s = length s `div` 2
      | otherwise = length s `div` 2 + 1
...
multiCompose fs = multiCompose' fs id

multiCompose' [] f = f
multiCompose' (f : fs) acc = multiCompose' fs (acc . f)
...
interpreter' [] _ = []
interpreter' (c : cs) point = case c of
  "up"     -> move [0, 1]
  "down"   -> move [0, -1]
  "left"   -> move [-1, 0]
  "right"  -> move [1, 0]
  "printX" -> print (show $ head point)
  "printY" -> print (show $ last point)
  _        -> interpreter' cs point
  where
    move delta = interpreter' cs (zipWith (+) point delta)
    print message = message : interpreter' cs point
