...
rectangle x0 y0 w h = Shape f
  where
    f (Coord x y) = x >= x0 && x < xMax && y >= y0 && y < yMax
    xMax = x0 + w
    yMax = y0 + h
...
paint (Picture f) (Shape g) (Picture h) = Picture j
  where
    j c
      | g c = f c
      | otherwise = h c
...
instance Transform Fill where
  apply (Fill color) (Picture f) = Picture (const color)
...
instance Transform Blur where
  apply Blur = averageColor

sumColors :: [Color] -> Color
sumColors = foldr1 (\(Color r1 g1 b1) (Color r2 g2 b2) -> Color (r1 + r2) (g1 + g2) (b1 + b2))

blendColors :: [Color] -> Color
blendColors cs =
  let Color r g b = sumColors cs
      count = length cs
   in Color (r `div` count) (g `div` count) (b `div` count)

averageColor (Picture f) = Picture g
  where
    g (Coord x y) =
      let up = Coord x (y + 1)
          down = Coord x (y - 1)
          ...
          right = Coord (x + 1) y
          centre = Coord x y
          colors = map f [centre, up, down, left, right]
       in blendColors colors
