...
average xs = getSum total / getSum count
  where
    (total, count) = foldMap (\x -> (Sum x, Sum 1)) xs
...
reverseNonEmpty (a :| []) = a :| []
reverseNonEmpty (a :| as) = head reversed :| (tail reversed ++ [a])
  where
    reversed = reverse as
...
instance (Ord a) => Semigroup (Set a) where
  Set as <> Set bs = case bs of
    [] -> Set as
    (b : bs) -> add b (Set as) <> Set bs
...
passwordAllowed p (ContainsSome cs) = any (`elem` p) cs
passwordAllowed p (DoesNotContain cs) = not $ passwordAllowed p (ContainsSome cs)
...
evaluate (Operation op a b) = case op of
