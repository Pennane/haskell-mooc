...
allEqual [] = True
allEqual [a] = True
allEqual (a : b : xs) = (a == b) && allEqual (b : xs)
...
longest :: (Ord a) => [[a]] -> [a]
longest = foldr1 pickLonger

pickLonger x y
  | length x > length y = x
  | length x < length y = y
  | otherwise = if head x < head y then x else y
...
freqs = foldr incOrOne Map.empty

incOrOne :: (Ord a) => a -> Map.Map a Int -> Map.Map a Int
incOrOne =
  Map.alter
    ( \m -> case m of
        Just a -> Just (a + 1)
...
transfer fromName toName amount bank
  | Map.notMember fromName bank = bank
  | Map.notMember toName bank = bank
  | amount < 0 = bank
  | currentBalance < amount = bank
  | otherwise = addToAccount . deductFromAccount $ bank
  where
    currentBalance = Map.findWithDefault 0 fromName bank
    deductFromAccount = Map.adjust (subtract amount) fromName
    addToAccount = Map.adjust (+ amount) toName
