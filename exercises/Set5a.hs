...
totalPrice (MkShoppingEntry _ x n) = x * fromIntegral n
...
buyOneMore (MkShoppingEntry s x n) = MkShoppingEntry s x (n + 1)
...
toNat z
  | z < 0 = Nothing
  | z == 0 = Just Zero
  | otherwise = fmap PlusOne (toNat (z - 1)) -- oh yeah functor mappin
